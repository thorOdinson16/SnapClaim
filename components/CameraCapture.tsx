...
import { processImageWithOCR } from '@/lib/image';

interface CameraCaptureProps {
  onCapture: (base64: string, ocrText: string) => void;
  onCancel: () => void;
}
...
export default function CameraCapture({ onCapture, onCancel }: CameraCaptureProps) {
  ...
  const handleFile = useCallback(async (file: File | null | undefined) => {
    ...
    setProcessing(true);
    try {
      const { base64, ocrText } = await processImageWithOCR(file);
      onCapture(base64, ocrText);
    } catch (err) {
    ...
    <div className="flex flex-col items-center gap-5 p-6 animate-fade-in">
      ...
      {!showFallback ? (
        <>
          ...
          <p className="text-gray-400 text-base text-center">Take a photo of the damaged item</p>
          <button onClick={triggerCamera} className="btn-primary w-full">
          ...
          <label className="w-full cursor-pointer bg-surface-200 hover:bg-surface-300 text-white font-medium py-3.5 px-6 rounded-2xl text-base transition-all duration-200 active:scale-[0.97] text-center border border-white/10">
            Choose from gallery
            <input ref={fallbackInputRef} type="file" accept="image/*" onChange={handleFallbackChange} className="hidden" />
          </label>
          <button onClick={onCancel} className="text-gray-400 hover:text-gray-200 text-sm underline underline-offset-4 transition-colors mt-1">Go back</button>
        </>
      ) : (
        <div className="flex flex-col items-center gap-4 text-center w-full max-w-xs animate-slide-up">
          ...
          <p className="text-gray-200 text-base leading-relaxed">Please allow camera access in your browser settings and try again.</p>
          <label className="w-full cursor-pointer bg-surface-200 hover:bg-surface-300 text-white font-medium py-3.5 px-6 rounded-2xl text-base transition-all duration-200 active:scale-[0.97] text-center border border-white/10">
            Choose from gallery
            <input type="file" accept="image/*" onChange={handleFallbackChange} className="hidden" />
          </label>
          <button onClick={onCancel} className="text-gray-400 hover:text-gray-200 text-sm underline underline-offset-4 transition-colors mt-1">Go back</button>
        </div>
      )}
